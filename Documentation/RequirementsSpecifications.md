...
Last Modified: 11/9/2024<br><br></h2>
...
* *Implementing a system for the FollowBot to handle fragile items or sort belongings into specific categories.*<br>
* *The ability for FollowBot to go upstairs, and go through tedious environment.*<br>
<br>
...
The webpage on hand will have some form of minor interaction with the FollowBot. The interaction will be a form of movement patterns. We plan on adding 2 specific interactions.
...
Another notable feature that will be implemented through the app is the ability to control the FollowBot. This will be meant for correcting its position towards the user, playing with the FollowBot.
...
6. **Mapping**: Using mapping APIs for pathfinding with FollowBot in order for it to go from location to location.<br><br>
...
| essential-backlog User Stories | Millstone | Assigned to: |
| --- | --- | --- |
| A user will be able to click tabs in the website in order to seek information about the FollowBot, how it performs and works. | M1  | Igor |   
...
| The application will constantly update to retrieve information from the Followbot about it's battery life, temperature, and weight | M8  | Frank |
...
| The FollowBot, in the event of getting stuck, will attempt to self-correct. If failed, the bot will notify the user and allow manual control.  | m14 | … |
...
| The user can be confident in the security of their data, through relying on the robust database, authentications, and opting out of data collection. | m16 | … |
